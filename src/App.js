...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, signInWithPopup, sendPasswordResetEmail, onAuthStateChanged, updateProfile, linkWithPopup, fetchSignInMethodsForEmail, EmailAuthProvider, linkWithCredential, verifyPasswordResetCode, confirmPasswordReset } from "firebase/auth";
...
// ── Auth ──
function Auth({ onDone, onBack, initialView="login", resetOobCode=null }) {
  ...
  // Verify the password-reset code from the email link before showing the new-password form
  useEffect(() => {
    if (view==="newpassword" && resetOobCode && !codeChecked) {
      verifyPasswordResetCode(auth, resetOobCode).then(emailForCode => {
        setResetEmail(emailForCode);
        setCodeChecked(true);
      }).catch(() => {
        setCodeErr("This reset link is invalid or has expired. Please request a new one.");
        setCodeChecked(true);
      });
    }
  }, [view, resetOobCode, codeChecked]);
  ...
  async function submit() {
    ...
    try {
      if (view==="newpassword") {
        ...
        await confirmPasswordReset(auth, resetOobCode, pass);
      ...
      const msg = e.code==="auth/invalid-credential"||e.code==="auth/user-not-found"||e.code==="auth/wrong-password" ? "Incorrect email or password. Please try again."
        ...
        : e.code==="auth/expired-action-code"||e.code==="auth/invalid-action-code" ? "This reset link is invalid or has expired. Please request a new one."
  ...
  if (view==="newpassword") {
    if (!codeChecked) return (
      <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 20px"}}>
        <span style={{width:28,height:28,border:`3px solid ${PINK}40`,borderTopColor:PINK,borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/>
      </div>
    );
    if (codeErr) return (
      <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 20px"}}>
        <div style={{background:"#fff",border:`1.5px solid ${BORDER}`,borderRadius:20,padding:"36px 28px",maxWidth:400,width:"100%",textAlign:"center"}}>
          <div style={{fontFamily:"Plus Jakarta Sans,sans-serif",fontWeight:900,fontSize:20,color:TEXT,marginBottom:8}}>Link expired</div>
          <div style={{fontSize:13,color:SUB,marginBottom:24,lineHeight:1.7}}>{codeErr}</div>
          <button onClick={()=>{ window.history.replaceState({},"","/login"); setView("forgot"); setCodeErr(""); setCodeChecked(false); }} style={{background:"none",border:"none",color:PINK,fontFamily:"Plus Jakarta Sans,sans-serif",fontWeight:700,fontSize:14,cursor:"pointer"}}>Request a new link</button>
        </div>
      </div>
    );
    return (
      <div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",fontFamily:"Poppins,sans-serif"}}>
        <div style={{background:"#fff",border:`1.5px solid ${BORDER}`,borderRadius:20,padding:"32px 28px",maxWidth:400,width:"100%"}}>
          ...
          <div style={{fontFamily:"Plus Jakarta Sans,sans-serif",fontWeight:900,fontSize:20,color:TEXT,marginBottom:4}}>Set new password</div>
          <div style={{fontSize:13,color:SUB,marginBottom:20}}>for <strong style={{color:TEXT}}>{resetEmail}</strong></div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{position:"relative"}}>
              <Inp placeholder="New password" value={pass} onChange={e=>setPass(e.target.value)} type={showP?"text":"password"} style={{paddingRight:56}}/>
            ...
            <Inp placeholder="Confirm new password" value={pass2} onChange={e=>setPass2(e.target.value)} type={showP?"text":"password"} onKeyDown={e=>e.key==="Enter"&&submit()}/>
            ...
            <PBtn full onClick={submit} disabled={busy}>
              {busy
                ? <span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><span style={{width:14,height:14,border:"2px solid rgba(255,255,255,.4)",borderTopColor:"#fff",borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/>Saving...</span>
                : "Save new password"
              }
            </PBtn>
          </div>
...
function ParticipantView({ session: init, hostPlan="free", onBack }) {
  ...
  if (step === "joined" && live?.boardVisible) return (
    <div style={{minHeight:"100vh",background:"#0D0008",fontFamily:"Poppins,sans-serif",color:"#fff",padding:"24px 20px",display:"flex",flexDirection:"column",alignItems:"center"}}>
      ...
      <BadgeClaimPrompt/>
      <Confetti active/>
      <img src="/Teticoin Logo Mark.png" alt="Teticoin" style={{width:56,height:56,objectFit:"contain"}}/>
  ...
  return <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center"}}><img src="/Teticoin Logo Mark.png" alt="Teticoin" style={{width:60,height:60,objectFit:"contain"}}/></div>;
}
    ...
    <div style={{minHeight:"100vh",background:"#0D0008",color:"#fff",fontFamily:"Poppins,sans-serif",padding:"28px 24px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:32}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <img src="/Teticoin Logo Mark.png" alt="Teticoin" style={{width:44,height:44,objectFit:"contain"}}/>
          <div>
            <img src="/Teticoin Logo Horizontal.png" alt="Teticoin" style={{height:28,width:"auto",filter:"brightness(0) invert(1)",opacity:0.9}}/>
            <div style={{fontSize:11,color:"rgba(255,255,255,.3)",letterSpacing:2,textTransform:"uppercase",marginTop:4}}>{session.name} · {session.code}</div>
          </div>
        </div>
      ...
      {proGateHint && (
        <div className="tc-modal-backdrop" style={{position:"fixed",inset:0,zIndex:800,backdropFilter:"blur(4px)",background:"rgba(26,10,20,.4)"}} onClick={()=>setProGateHint(null)}>
          <div className="tc-modal-sheet" style={{background:"#fff",padding:"28px 24px 32px",width:"100%",position:"relative"}} onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={()=>{ setProGateHint(null); if(onUpgrade) onUpgrade(); }} style={{width:"100%",padding:"14px 0",background:GRAD,border:"none",borderRadius:13,fontFamily:"Plus Jakarta Sans,sans-serif",fontWeight:800,fontSize:15,color:"#fff",cursor:"pointer",marginBottom:10}}>Upgrade to Pro · RM {PRO_PRICE_RM.monthly}/mo</button>
    ...
    <div onClick={onUpgrade}
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:11,color:"#2563EB",marginTop:1,fontWeight:500}}>
          {atLimit?`Upgrade to Pro — unlimited sessions from RM ${PRO_PRICE_RM.monthly}/mo`:"Upgrade for unlimited sessions"}
        </div>
      </div>
    ...
    <div className="tc-modal-backdrop" style={{position:"fixed",inset:0,zIndex:800,backdropFilter:"blur(4px)",background:"rgba(26,10,20,.4)"}}>
      <div className="tc-modal-sheet" style={{background:"#fff",padding:"28px 24px 36px",width:"100%"}}>
        ...
        <button onClick={onUpgrade} style={{width:"100%",padding:"14px 0",background:GRAD,border:"none",borderRadius:13,fontFamily:"Plus Jakarta Sans,sans-serif",fontWeight:800,fontSize:15,color:"#fff",cursor:"pointer",marginBottom:10}}>
          {cfg.cta} · from RM {PRO_PRICE_RM.monthly}/mo
        </button>
...
function BillingPage({ plan="free", planExpiry:planExpiryProp=null, sessionCount=0, maxPax=0, trainer=null, onUpgrade, onClose }) {
  ...
  const planData = {
    ...
    oneTime: {name:"One Time",    price:`RM ${PRO_PRICE_RM.monthly}`,   renewal:"one-time", color:BLUE,   next:null},
    pro:     {name:"Pro",         price:`RM ${PRO_PRICE_RM.monthly}`,   renewal:"monthly",  color:BLUE,   next:expiryLabel},
    proY:    {name:"Pro",         price:`RM ${PRO_PRICE_RM.yearly}`,  renewal:"yearly",   color:BLUE,   next:expiryLabel},
                ...
                <>
                  ...
                  {isExpiringSoon && (
                    <button onClick={()=>{ onClose(); setTimeout(()=>onUpgrade(),100); }}
                      ...
                      Upgrade to Pro — RM {PRO_PRICE_RM.monthly}/mo
                    </button>
                  )}
                </>
              ) : isFree ? (
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      ...
                      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                        <button onClick={()=>startExtend("monthly")} disabled={!!extending}
                          ...
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
                          {extending==="monthly" ? "Starting…" : `${isExpired ? "Reactivate" : "Extend"} 1 month — RM ${PRO_PRICE_RM.monthly}`}
                        </button>
                        <button onClick={()=>startExtend("yearly")} disabled={!!extending}
                          ...
                          {extending==="yearly" ? "Starting…" : <>Extend 1 year — RM {PRO_PRICE_RM.yearly} <span style={{fontSize:10,opacity:.85}}>(save RM {PRO_YEARLY_SAVINGS})</span></>}
                        </button>
                      </div>
                      <button onClick={()=>{ onClose(); setTimeout(()=>onUpgrade(),100); }}
...
function SuperAdminDashboard({ onClose }) {
  ...
  const adminCSS = `
    ...
    .tc-card-info { position:absolute; top:10px; right:10px; cursor:default; }
    .tc-card-info svg { display:block; opacity:.45; }
    .tc-card-tooltip { display:none; position:absolute; top:18px; right:0; width:180px; background:#1A1A2E; color:#fff; font-size:10px; line-height:1.55; border-radius:8px; padding:8px 10px; z-index:999; pointer-events:none; box-shadow:0 4px 16px rgba(0,0,0,.18); font-family:'DM Sans',sans-serif; font-weight:400; }
    .tc-card-info:hover .tc-card-tooltip { display:block; }
  `;
            ...
            <div style={{background:"#fff",border:`1.5px solid ${BORDER}`,borderRadius:14,padding:"18px 20px",display:"flex",flexDirection:"column",gap:4,position:"relative"}}>
              {tooltip && (
                <div className="tc-card-info">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={SUB} strokeWidth="2.2" strokeLinecap="round">
                  ...
                  <>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:12}}>
                      <Card label="Total Sessions" value={usageData.totalSessions.toLocaleString()} color={PINK}
                        tooltip="Total sessions ever created across all hosts, including archived ones."
                      ...
                      <Card label="Total Participants" value={usageData.totalParticipants.toLocaleString()} sub={usageData.totalSessions?`avg ${(usageData.totalParticipants/usageData.totalSessions).toFixed(1)}/session`:""} color={PURPLE}
                        tooltip="Sum of all participants across all sessions. One person joining 3 sessions counts as 3. Avg shows typical room size."
                      ...
                      <Card label="Total Coins Awarded" value={usageData.totalCoins.toLocaleString()} color="#F59E0B"
                        tooltip="Total coin points awarded across all sessions. Reflects overall engagement volume on the platform."
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:12}}>
                      <Card label="Total Groups Created" value={usageData.totalGroups.toLocaleString()} sub={usageData.totalSessions?`avg ${(usageData.totalGroups/usageData.totalSessions).toFixed(1)}/session`:""} color={GREEN}
                        tooltip="Total team/group objects created across all sessions. Higher avg means hosts regularly use the team competition feature."
                        icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>}/>
                      <Card label="Avg Coin per Participant" value={usageData.totalParticipants?Math.round(usageData.totalCoins/usageData.totalParticipants).toLocaleString():"—"} sub="average across all sessions" color={GREEN}
                        tooltip="Total coins ÷ total participants. A higher number means hosts award frequently — a strong signal of active, engaged sessions."
                        icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>}/>
                    </div>
...
export default function App() {
  ...
  const [resetOobCode, setResetOobCode] = useState(null); // oobCode from Firebase password-reset email link
  ...
  useEffect(() => {
    ...
    // Password-reset link from Firebase Auth email (?mode=resetPassword&oobCode=...)
    if (qs.get("mode") === "resetPassword" && qs.get("oobCode")) {
      setResetOobCode(qs.get("oobCode"));
      setAuthInitialView("newpassword");
      setScreen("auth");
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (qs.get("mode") === "resetPassword" && qs.get("oobCode")) return; // skip — password-reset screen already set above
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (currentQs.get("mode") === "resetPassword" && currentQs.get("oobCode")) return;
  ...
  if (screen==="auth") return <><style>{CSS}</style><Auth onDone={handleAuth} onBack={()=>{ window.history.replaceState({},"","/"); setScreen("landing"); }} initialView={authInitialView} resetOobCode={resetOobCode}/></>;
    ...
    <div className="tc-app-shell" style={{minHeight:"100vh",background:BG,fontFamily:"Poppins,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div className="tc-home-wrap" style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,.55)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
            <div style={{background:"#fff",borderRadius:24,padding:"36px 28px",maxWidth:420,width:"100%",textAlign:"center",boxShadow:"0 24px 80px rgba(0,0,0,.2)",animation:"fadeIn .3s ease"}}>
              ...
              <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:"12px 16px",marginBottom:24,fontSize:13,color:"#166534",lineHeight:1.65}}>
                To keep enjoying unlimited sessions, up to 200 participants, groups, Coinmaster and all the features you loved — upgrade to <strong>Pro</strong> from just <strong>RM {PRO_PRICE_RM.monthly}/mo</strong>.
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div className="tc-home-topnav" style={{display:"none",background:plan==="superadmin"?"linear-gradient(90deg,#ffffff 0%,#ffffff 35%,#EF4444 75%,#B91C1C 100%)":plan==="beta"?"linear-gradient(90deg,#ffffff 0%,#ffffff 35%,#06B6D4 70%,#16A34A 100%)":plan!=="free"?"linear-gradient(90deg,#ffffff 0%,#ffffff 35%,#06B6D4 75%,#1D4ED8 100%)":"#fff",borderBottom:plan==="superadmin"?"1px solid rgba(239,68,68,0.25)":plan==="beta"?"1px solid rgba(6,182,212,0.25)":plan!=="free"?"1px solid rgba(6,182,212,0.25)":`1px solid ${BORDER}`,padding:"0 32px",height:64,alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <img src="/Teticoin Logo Horizontal.png" alt="Teticoin" style={{height:32,width:"auto"}}/>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,
...
const CSS = `
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  @media (max-width: 899px) {
    ...
    .tc-admin-btns-desktop { display: none !important; }
    .tc-admin-btns-mobile  { display: block !important; }
  }
  /* Global cursor fix */
  * { caret-color: #1A0A14 !important; }
