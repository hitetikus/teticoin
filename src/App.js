...
function HamLoading() {
  ...
  useEffect(() => {
    function init() {
      if (!coinRef.current || !window.lottie) return;
      if (lottieRef.current) return; // already initialized
      lottieRef.current = window.lottie.loadAnimation({
        container: coinRef.current,
        renderer: "svg",
        loop: true,
        autoplay: true,
        animationData: COIN_LOTTIE_DATA,
      });
    }
    ...
    return () => { if (lottieRef.current) { lottieRef.current.destroy(); lottieRef.current = null; } };
  }, []);

  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:0}}>
      {/* Scrolling coins via lottie */}
      <div style={{display:"flex",gap:0}}>
        {[0,1,2].map(i=>(
          <div key={i} ref={i===0?coinRef:null} style={{
            width:54,height:54,
            animation:`coinFlyIn 1.2s ease-in-out ${i*0.28}s infinite`,
          }}>
            {i===0 ? null : <LottieCoin/>}
          </div>
        ))}
      </div>
      {/* Hamster face — chomping */}
      <div style={{animation:"hamBob 0.4s ease-in-out infinite alternate",marginLeft:-6,zIndex:2}}>
        <svg width="90" height="90" viewBox="0 0 80 80" fill="none">
          {/* Left ear */}
          <ellipse cx="18" cy="20" rx="13" ry="13" fill="#F9A8D4"/>
          <ellipse cx="18" cy="20" rx="7" ry="7" fill="#FDE8F0"/>
          {/* Right ear */}
          <ellipse cx="62" cy="20" rx="13" ry="13" fill="#F9A8D4"/>
          <ellipse cx="62" cy="20" rx="7" ry="7" fill="#FDE8F0"/>
          {/* Body */}
          <ellipse cx="40" cy="56" rx="28" ry="22" fill="#FDE8F0"/>
          <ellipse cx="40" cy="40" rx="30" ry="28" fill="#FCE7F3"/>
          <ellipse cx="40" cy="60" rx="17" ry="13" fill="#FFF0F5"/>
          {/* Puffy coin-stuffed cheeks */}
          <ellipse cx="14" cy="44" rx="12" ry="11" fill="#FDA4CF" opacity=".95"/>
          <ellipse cx="66" cy="44" rx="12" ry="11" fill="#FDA4CF" opacity=".95"/>
          {/* Coin glint in cheek */}
          <ellipse cx="12" cy="42" rx="5" ry="4" fill="#F5A623" opacity=".4"/>
          {/* Happy squint eyes */}
          <path d="M27 32 Q32 27 37 32" stroke="#1A0A14" strokeWidth="2.8" fill="none" strokeLinecap="round"/>
          <path d="M43 32 Q48 27 53 32" stroke="#1A0A14" strokeWidth="2.8" fill="none" strokeLinecap="round"/>
          {/* Nose */}
          <ellipse cx="40" cy="42" rx="3.2" ry="2.2" fill="#E91E8C"/>
          {/* Whiskers */}
          <line x1="43" y1="42" x2="64" y2="37" stroke="#FDA4CF" strokeWidth="1.1" opacity=".8"/>
          <line x1="43" y1="44" x2="64" y2="46" stroke="#FDA4CF" strokeWidth="1.1" opacity=".8"/>
          <line x1="37" y1="42" x2="16" y2="37" stroke="#FDA4CF" strokeWidth="1.1" opacity=".8"/>
          <line x1="37" y1="44" x2="16" y2="46" stroke="#FDA4CF" strokeWidth="1.1" opacity=".8"/>
          {/* Mouth — upper fixed, lower chomps */}
          <path d="M26 50 Q40 46 54 50" fill="#FCE7F3" stroke="#FDA4CF" strokeWidth="1"/>
          <g style={{transformOrigin:"40px 50px",animation:"chomp 0.3s ease-in-out infinite alternate"}}>
            <path d="M26 50 Q40 62 54 50" fill="#FFF0F5" stroke="#FDA4CF" strokeWidth="1"/>
            <ellipse cx="40" cy="55" rx="5" ry="3.2" fill="#F472B6"/>
          </g>
          {/* Tiny paws */}
          <ellipse cx="22" cy="72" rx="9" ry="6" fill="#FDE8F0"/>
          <ellipse cx="58" cy="72" rx="9" ry="6" fill="#FDE8F0"/>
          <ellipse cx="22" cy="73" rx="5.5" ry="3.5" fill="#FECDE8"/>
          <ellipse cx="58" cy="73" rx="5.5" ry="3.5" fill="#FECDE8"/>
        </svg>
      </div>
    </div>
  );
...
// Standalone lottie coin — for copies 1 and 2 in the row
function LottieCoin() {
  ...
  return <div ref={ref} style={{width:54,height:54}}/>;
}
// ── Confetti ──
...
function MassGive({ participants, groups, onAward, onClose }) {
  ...
  const finalAmtRef = useRef(finalAmt);
  ...
  function startScanner() {
    if (!ok) return;
    setScannerErr("");
    scanLogRef.current = []; // reset dedup list on each new scan session
    setScanning(true);
  }
  ...
  useEffect(() => {
    ...
    function initScanner() {
      ...
      scanner.start(
        ...
        (decodedText) => {
          // decodedText = "P001" or "001" — read from real QR generated by PQR component
          const raw = decodedText.replace(/^P/i,"").trim();
          const num = parseInt(raw);
          if (isNaN(num)) return;
          const p = participantsRef.current.find(x => x.num === num);
          if (!p) return;
          if (scanLogRef.current.find(l => l.id === p.id)) return; // already scanned this session
...
export default function App() {
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      ...
      <div style={{fontSize:13,color:SUB,fontFamily:"Poppins,sans-serif",marginTop:-6}}>Nomming coins, hang tight…</div>
    </div>
  );
