...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, signInWithPopup, sendPasswordResetEmail, onAuthStateChanged, updateProfile, linkWithPopup, fetchSignInMethodsForEmail, EmailAuthProvider, linkWithCredential, setPersistence, browserLocalPersistence } from "firebase/auth";
      ...
      <div className="tc-modal-sheet" style={{background:"#fff",maxHeight:"90vh",display:"flex",flexDirection:"column",animation:"slideUp .25s ease"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 20px 32px"}}>
          ...
          <input type="number" placeholder="Custom amount…" value={cAmt} onChange={e=>{setCAmt(e.target.value);setAmt("custom");}}
            style={{width:"100%",background:BG,border:`1.5px solid ${amt==="custom"?PINK:BORDER}`,borderRadius:12,padding:"10px 14px",fontFamily:"Poppins,sans-serif",fontSize:13,color:TEXT,outline:"none",marginBottom:16}}/>
...
function ParticipantView({ session: init, hostPlan="free", onBack }) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      ...
      if (!s) return; // keep showing last good state — don't reset to name screen
      // Guard: don't overwrite local state with stale Firestore data
      // (can happen if a host write is in-flight and Firestore returns an older snapshot)
      if (s.updatedAt && live?.updatedAt && s.updatedAt < live.updatedAt) return;
      if (myId) {
...
function Session({ session: init, plan="free", paxLimit=FREE_PAX_LIMIT, sessionCount=0, allSessions=[], onDuplicateSession, onBack, onPView, onUpgrade }) {
  ...
  const [pMenuPos, setPMenuPos]   = useState({top:0,bottom:undefined,right:0}); // fixed position of ⋯ dropdown
  ...
  function mut(fn) {
    setSes(prev => {
      const s = JSON.parse(JSON.stringify(prev));
      fn(s);
      s.updatedAt = Date.now(); // stamp every write so participant polls can detect freshness
  ...
  return (
    <div className="tc-app-shell">
      ...
      {pMenuOpen && (() => {
        ...
        if (!p) return null;
        return (
          <div style={{position:"fixed",top:pMenuPos.top,bottom:pMenuPos.bottom,right:pMenuPos.right,background:"#fff",border:`1.5px solid ${BORDER}`,borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,.14)",zIndex:9990,minWidth:170,overflow:"hidden"}}
                      ...
                      {isEditing ? (
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px"}}>
                          ...
                          <div style={{position:"relative",flexShrink:0}}>
                            <button onClick={e=>{e.stopPropagation();if(menuOpen){setPMenuOpen(null);}else{const r=e.currentTarget.getBoundingClientRect();const menuH=130;const flipUp=r.bottom+menuH>window.innerHeight-20;setPMenuPos(flipUp?{bottom:window.innerHeight-r.top+4,right:window.innerWidth-r.right}:{top:r.bottom+4,right:window.innerWidth-r.right});setPMenuOpen(p.id);}}}
...
const BULB_LOTTIE_B64 = "";
...
// ── Root app ──
// Load dotlottie player for animated bulb icon
if (typeof window !== "undefined" && !document.querySelector("script[data-dotlottie]")) {
  const _s = document.createElement("script");
  _s.src = "https://unpkg.com/@dotlottie/player-component@latest/dist/dotlottie-player.js";
  _s.setAttribute("data-dotlottie", "1");
...
export default function App() {
  ...
  const paxLimit = isFree ? FREE_PAX_LIMIT : 99999;
  ...
  useEffect(() => {
    ...
    // Ensure auth session persists across page reloads (fixes Google login getting dropped)
        ...
        <div className="tc-home-inner">
          ...
          <div className="tc-home-left" style={{padding:"20px"}}>
            ...
            <div className="tc-home-mobile-topbar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span onClick={()=>setShowTips(true)} data-tip="Host Tips" data-tip-below="" style={{cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"transform .25s cubic-bezier(0.22,1,0.36,1)"}} onMouseEnter={e=>e.currentTarget.style.transform="translateY(-5px)"} onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}><svg width="22" height="22" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m408 176a152 152 0 1 0 -231.058 129.836 31.744 31.744 0 0 1 15.058 27.145v43.019h128v-43.023a32.1 32.1 0 0 1 15.384-27.341 151.893 151.893 0 0 0 72.616-129.636z" fill="#ffe76c"/><path d="m320 408h-128a16 16 0 0 1 -16-16 16 16 0 0 1 16-16h128a16 16 0 0 1 16 16 16 16 0 0 1 -16 16z" fill="#dadcde"/><path d="m320 440h-128a16 16 0 0 1 -16-16 16 16 0 0 1 16-16h128a16 16 0 0 1 16 16 16 16 0 0 1 -16 16z" fill="#e9eef2"/><path d="m192 440h128a0 0 0 0 1 0 0v16a32 32 0 0 1 -32 32h-64a32 32 0 0 1 -32-32v-16a0 0 0 0 1 0 0z" fill="#dadcde"/><path d="m320 376h-128a16 16 0 0 1 -16-16 16 16 0 0 1 16-16h128a16 16 0 0 1 16 16 16 16 0 0 1 -16 16z" fill="#e9eef2"/><path d="m256.011 16h-.011a160.035 160.035 0 0 0 -138.868 239.516 161.029 161.029 0 0 0 55.644 57.149 23.857 23.857 0 0 1 11.224 20.315v4.4a23.977 23.977 0 0 0 -9.869 38.62 23.943 23.943 0 0 0 0 32 23.977 23.977 0 0 0 9.869 38.624v9.376a40.045 40.045 0 0 0 40 40h64a40.045 40.045 0 0 0 40-40v-9.376a23.977 23.977 0 0 0 9.869-38.624 23.943 23.943 0 0 0 0-32 23.977 23.977 0 0 0 -9.869-38.624v-4.4a23.955 23.955 0 0 1 11.568-20.523 159.891 159.891 0 0 0 76.432-136.453c0-88.218-71.771-159.994-159.989-160zm-64.011 336h128a8 8 0 0 1 0 16h-128a8 8 0 0 1 0-16zm40-96h-8a8 8 0 1 1 8-8zm16 16h16v64h-16zm80 120a8.009 8.009 0 0 1 -8 8h-128a8 8 0 0 1 0-16h128a8.009 8.009 0 0 1 8 8zm-40 88h-64a24.027 24.027 0 0 1 -24-24v-8h112v8a24.027 24.027 0 0 1 -24 24zm32-48h-128a8 8 0 0 1 0-16h128a8 8 0 0 1 0 16zm11.2-133.183a39.85 39.85 0 0 0 -19.2 34.16v3.023h-32v-64h8a24 24 0 1 0 -24-24v8h-16v-8a24 24 0 1 0 -24 24h8v64h-32v-3.02a39.95 39.95 0 0 0 -18.891-33.973 143.982 143.982 0 0 1 74.891-267.007h.011c79.395.006 143.989 64.6 143.989 144a143.156 143.156 0 0 1 -68.8 122.817zm-51.2-42.817v-8a8 8 0 1 1 8 8z"/><path d="m256 48h-8a8 8 0 0 0 0 16h8a112.127 112.127 0 0 1 112 112v8a8 8 0 0 0 16 0v-8a128.145 128.145 0 0 0 -128-128z"/></svg></span>
              <button onClick={()=>setProfileOpen(v=>!v)}
